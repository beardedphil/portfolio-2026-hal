#!/usr/bin/env node
/**
 * Cleanup duplicate artifacts for tickets by directly updating each artifact type once.
 * This triggers the duplicate deletion logic in the HAL API endpoints.
 * Usage: node scripts/cleanup-duplicate-artifacts-v2.js <ticketId1> [ticketId2] ...
 */
...
if (ticketIds.length === 0) {
  console.error('Usage: node scripts/cleanup-duplicate-artifacts-v2.js <ticketId1> [ticketId2] ...')
  process.exit(1)
}
...
async function cleanupTicket(ticketId) {
  ...
  // For each type, find the best artifact and update it once
  // This single update should trigger the duplicate deletion logic
  for (const [type, typeArtifacts] of byType.entries()) {
    ...
    console.log(`  🔍 Processing ${type}: ${typeArtifacts.length} duplicates`)

    // Find the best artifact to keep (most recent with content)
    ...
    console.log(`    Will delete ${duplicatesCount} duplicate(s) via update`)
    ...
    try {
      ...
      if (updateData.success) {
        ...
        console.log(`    ✅ Updated and cleaned up ${updateData.cleaned_up_duplicates || duplicatesCount} duplicate(s)`)
      } else {
...
async function main() {
  ...
  for (const ticketId of ticketIds) {
    ...
    // Wait a bit between tickets to avoid rate limiting
