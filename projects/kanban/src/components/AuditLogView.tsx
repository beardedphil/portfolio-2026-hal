...
export function AuditLogView({
  ...
  // Redact sensitive values from metadata to prevent secret leakage
  const redactMetadata = (metadata: Record<string, unknown>): Record<string, unknown> => {
    const REDACTED = '[REDACTED]'
    const SENSITIVE_KEYS = /^(api[_-]?key|apikey|authorization|secret|password|token|supabase[_-]?(anon|service)[_-]?key|access[_-]?token|refresh[_-]?token)$/i
    ...
    const redactValue = (value: unknown, key?: string): unknown => {
      ...
      if (typeof value === 'string') {
        return value.replace(OPENAI_KEY_PATTERN, REDACTED).replace(JWT_PATTERN, REDACTED)
      }
