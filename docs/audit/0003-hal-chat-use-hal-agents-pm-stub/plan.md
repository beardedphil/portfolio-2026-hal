...
Replace HAL's inline PM chat stub with a call into the `portfolio-2026-hal-agents` (submodule at `projects/project-1`) Project Manager module so the response comes from the agents repo and is visibly different (e.g. `[PM@hal-agents]`). Diagnostics must show PM implementation source and last agent error.

## Approach

### 1. PM Module in hal-agents (projects/project-1)

- Add minimal PM stub in submodule at `projects/project-1/src/agents/projectManager.ts`.
...
- Add Vite `resolve.alias`: `@hal-agents` → `projects/project-1/src`.
