...
**When to use:** For any ticket that modifies chat functionality, message handling, or conversation state management.

**Steps:**
1. **Start a chat**: Connect to a repo and send at least one message in any agent chat (PM, Implementation, or QA).
2. **Disconnect/reconnect**: Click "Disconnect" button, then reconnect to the same repo (or refresh the page).
3. **Verify thread and messages remain visible**: Open the same chat thread and confirm:
   - The same thread ID is visible (if shown in UI)
   - All previous messages are still present
   - Message count is unchanged
   - No empty message shells or placeholders appear
4. **Send a new message after reconnect**: Type and send a new message in the same chat thread.
5. **Verify no duplicates/blank messages**: Confirm:
   - No duplicate messages appear
   - No blank/empty messages are created
   - The new message appears correctly in the conversation flow
   - Agent status boxes (if applicable) return with correct state

**Expected UI results:**
- Same thread ID persists across disconnect/reconnect
- Message count unchanged after reconnect
- All conversation history visible and intact
- New messages append correctly without duplicates
- No empty shells or placeholder messages
- Agent status boxes restore correctly (if applicable)
