/**
 * Instruction loading logic extracted from projectManager.ts to improve maintainability.
 */
...
function mapHalInstruction(raw: Record<string, unknown>): AgentInstruction {
...
function dedupeTopics(topics: Array<{ topicId: string; title: string; description: string }>) {
...
async function getHalBaseUrl(config: PmAgentConfig): Promise<string> {
  let halBaseUrl: string = (process.env.HAL_API_BASE_URL || 'https://portfolio-2026-hal.vercel.app').trim()
  try {
    ...
    // .hal/api-base-url missing, use default
  }
  return halBaseUrl
}

export async function loadInstructionSetByAgentType(
  config: PmAgentConfig,
  targetAgentType: 'project-manager' | 'implementation-agent' | 'qa-agent' | 'process-review-agent'
) {
  ...
  const halBaseUrl = await getHalBaseUrl(config)
...
export async function loadInstructionTopicById(config: PmAgentConfig, topicId: string) {
  const halBaseUrl = await getHalBaseUrl(config)
  const repoFullName = config.repoFullName || config.projectId || 'beardedphil/portfolio-2026-hal'
