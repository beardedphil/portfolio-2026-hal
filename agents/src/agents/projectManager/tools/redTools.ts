/**
 * RED (Requirement Expansion Document) tools for PM agent.
 */
...
export interface RedToolsDeps {
  toolCalls: ToolCallRecord[]
  halBaseUrl: string
  config: { abortSignal?: AbortSignal; onProgress?: (message: string) => void | Promise<void> }
...
/**
 * Create RED document tools.
 */
export function createRedTools(deps: RedToolsDeps) {
  const { toolCalls, halBaseUrl, config } = deps
    ...
    execute: async (input: CreateRedDocumentInput) => {
      ...
      try {
        const { json: fetched } = await halFetchJson(halBaseUrl, '/api/tickets/get', { ticketId: input.ticket_id }, {
          timeoutMs: 20_000,
          progressMessage: `Fetching ticket ${input.ticket_id} for RED…`,
        ...
        const { json: existing } = await halFetchJson(halBaseUrl, '/api/red/list', { ticketPk, repoFullName }, {
          timeoutMs: 20_000,
          progressMessage: `Checking existing REDs for ${input.ticket_id}…`,
          abortSignal: config.abortSignal,
          onProgress: config.onProgress,
        })
        if (existing?.success && Array.isArray(existing.red_versions) && existing.red_versions.length > 0) {
          ...
          try {
            await halFetchJson(
              ...
              {
                timeoutMs: 20_000,
                progressMessage: `Validating existing RED for ${input.ticket_id}…`,
                abortSignal: config.abortSignal,
                onProgress: config.onProgress,
              }
            )
          ...
          try {
            ...
            const { json: redGet } = await halFetchJson(
              ...
              {
                timeoutMs: 20_000,
                progressMessage: `Loading latest RED JSON for ${input.ticket_id}…`,
                abortSignal: config.abortSignal,
                onProgress: config.onProgress,
              }
            )
...
`
              await halFetchJson(
                ...
                {
                  timeoutMs: 25_000,
                  progressMessage: `Saving RED artifact for ${input.ticket_id}…`,
                  abortSignal: config.abortSignal,
                  onProgress: config.onProgress,
                }
              )
        ...
        const { json: created } = await halFetchJson(
          ...
          {
            timeoutMs: 25_000,
            progressMessage: `Creating RED for ${input.ticket_id}…`,
            abortSignal: config.abortSignal,
            onProgress: config.onProgress,
          }
        )
        if (!created?.success || !created?.red_document) {
          ...
          try {
            await halFetchJson(
              ...
              {
                timeoutMs: 20_000,
                progressMessage: `Validating RED for ${input.ticket_id}…`,
                abortSignal: config.abortSignal,
                onProgress: config.onProgress,
              }
            )
...
`
  ...
  return {
    create_red_document_v2: createRedDocumentTool,
  }
}
