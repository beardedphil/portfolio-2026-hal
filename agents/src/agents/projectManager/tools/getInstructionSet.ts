/**
 * getInstructionSet tool implementation extracted from projectManager.ts
 * to improve maintainability by reducing file size and complexity.
 */
...
/**
 * Creates the get_instruction_set tool.
 * This tool loads instruction content from HAL/Supabase.
 */
export function createGetInstructionSetTool(
    ...
    execute: async (input) => {
      try {
        ...
        if (input.agentType) {
          ...
          let apiBaseUrl: string | null = halBaseUrl
          try {
            ...
            if (candidate) apiBaseUrl = candidate
          } catch {
          ...
          if (apiBaseUrl) {
            try {
              const [basicRes, situationalRes] = await Promise.all([
                fetch(`${apiBaseUrl}/api/instructions/get`, {
                  method: 'POST',
        ...
        try {
          const apiBaseUrlPath = path.join(config.repoRoot, '.hal', 'api-base-url')
          const apiBaseUrlContent = await fs.readFile(apiBaseUrlPath, 'utf8')
          apiBaseUrl = apiBaseUrlContent.trim()
        } catch {
          // .hal/api-base-url not found
        }

        if (!apiBaseUrl) {
          apiBaseUrl = halBaseUrl
        }

        if (apiBaseUrl) {
          try {
            const res = await fetch(`${apiBaseUrl}/api/instructions/get-topic`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                topicId,
