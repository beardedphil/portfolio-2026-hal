/**
 * Repository/file access tools for PM agent.
 */

import { tool } from 'ai'
import { z } from 'zod'
import { listDirectory, readFile, searchFiles, type ToolContext } from '../../tools.js'
import type { ToolCallRecord } from '../../projectManager.js'
import type { PmAgentConfig } from '../contextBuilding.js'

export interface RepositoryToolsDeps {
  toolCalls: ToolCallRecord[]
  ctx: ToolContext
  config: Pick<PmAgentConfig, 'repoFullName' | 'githubReadFile' | 'githubSearchCode' | 'githubListDirectory'>
  repoUsage: Array<{ tool: string; usedGitHub: boolean; path?: string }>
}

/**
 * Create repository/file access tools.
 */
export function createRepositoryTools(deps: RepositoryToolsDeps) {
  const { toolCalls, ctx, config, repoUsage } = deps
    ...
    execute: async (input) => {
      ...
      if (hasGitHubRepo && config.githubReadFile) {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            `[PM Agent] Falling back to HAL repo for: ${input.path} (hasGitHubRepo=${hasGitHubRepo}, hasGithubReadFile=${typeof config.githubReadFile === 'function'})`
          )
        }
        out = await readFile(ctx, input)
      }
      toolCalls.push({ name: 'read_file', input, output: out })
      return typeof (out as { error?: string }).error === 'string' ? JSON.stringify(out) : out
    },
  })
  ...
  const searchFilesTool = tool({
    ...
    execute: async (input) => {
      ...
      if (hasGitHubRepo && config.githubSearchCode) {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            `[PM Agent] Falling back to HAL repo for search: ${input.pattern} (hasGitHubRepo=${hasGitHubRepo}, hasGithubSearchCode=${typeof config.githubSearchCode === 'function'})`
          )
        }
  ...
  const listDirectoryTool = tool({
    ...
    execute: async (input) => {
      ...
      if (hasGitHubRepo && config.githubListDirectory) {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn(
            `[PM Agent] Falling back to HAL repo for list_directory: ${input.path} (hasGitHubRepo=${hasGitHubRepo}, hasGithubListDirectory=${typeof config.githubListDirectory === 'function'})`
          )
        }
