...
/**
 * Create kanban-related tools.
 */
export function createKanbanTools(deps: KanbanToolsDeps) {
    ...
    execute: async (input: { ticket_id: string; position: 'top' | 'bottom' | null }) => {
      ...
      try {
        const { json: fetched } = await halFetchJson(halBaseUrl, '/api/tickets/get', { ticketId: input.ticket_id }, {
          timeoutMs: 20_000,
          progressMessage: `Checking current column for ${input.ticket_id}…`,
        ...
        const { json: moved } = await halFetchJson(
          ...
          {
            timeoutMs: 25_000,
            progressMessage: `Moving ${ticketIdToMove} to To Do…`,
            abortSignal: config.abortSignal,
            onProgress: config.onProgress,
          }
        )
      ...
      try {
        const columnName =
          typeof input.column_name === 'string' && input.column_name.trim().length > 0
            ...
            : undefined
        const columnId =
          typeof input.column_id === 'string' && input.column_id.trim().length > 0 ? input.column_id : undefined
        const position = input.position ?? undefined

        const response = await fetch(`${halBaseUrl}/api/tickets/move`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
