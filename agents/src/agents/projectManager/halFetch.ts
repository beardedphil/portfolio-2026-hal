/**
 * HAL API fetch helper with timeout and abort signal support.
 */
...
export interface HalFetchConfig {
  halBaseUrl: string
...
export async function halFetchJson(
  path: string,
  body: unknown,
  opts: HalFetchOptions | undefined,
  config: HalFetchConfig
  ...
  try {
    ...
    const res = await fetch(`${config.halBaseUrl}${path}`, {
      method: 'POST',
