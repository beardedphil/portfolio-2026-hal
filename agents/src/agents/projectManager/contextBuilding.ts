...
// Extract local file loading logic
...
// Extract conversation formatting logic
...
// Extract instruction bootstrap appending logic
...
// Extract HAL API instruction loading logic
async function loadHalInstructions(
  sections: string[],
  config: PmAgentConfig,
  rulesPath: string
...
// Extract git status section
async function addGitStatusSection(sections: string[], repoRoot: string): Promise<void> {
...
export async function buildContextPack(config: PmAgentConfig, userMessage: string): Promise<string> {
  ...
  const localRules = await loadLocalRules(config.repoRoot, rulesPath)
  const localLoaded = localRules.success
  let ticketTemplateContent = localRules.ticketTemplateContent
  let checklistContent = localRules.checklistContent
  ...
  const conversationResult = formatConversationSection(config, userMessage)
  ...
  if (!localLoaded) {
    const halInstructions = await loadHalInstructions(sections, config, rulesPath)
    if (halInstructions.ticketTemplateContent) ticketTemplateContent = halInstructions.ticketTemplateContent
    if (halInstructions.checklistContent) checklistContent = halInstructions.checklistContent
  }
  ...
  await addGitStatusSection(sections, config.repoRoot)
