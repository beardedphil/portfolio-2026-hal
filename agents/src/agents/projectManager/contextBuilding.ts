...
async function tryLoadLocalRules(
  ...
  try {
    ...
    if (templateContent && checklistRead && agentInstructions) {
      ...
      const halContractPath = path.join(
        repoRoot,
...
function addInstructionsHeader(sections: string[], localLoaded: boolean): void {
  if (localLoaded) {
    sections.push(
      '## Instructions\n\n' +
...
function addTicketTemplateSection(
  sections: string[],
  localLoaded: boolean,
  ticketTemplateContent: string | null,
  checklistContent: string | null
...
export async function buildContextPack(config: PmAgentConfig, userMessage: string): Promise<string> {
  ...
  const localLoadResult = await tryLoadLocalRules(config.repoRoot, rulesPath)
  let { localLoaded, ticketTemplateContent, checklistContent, localRulesContent } = localLoadResult

  addInstructionsHeader(sections, localLoaded)
  addConversationSection(sections, config, userMessage)

  if (localLoaded) {
    sections.push('## Repo rules (local)\n\n' + localRulesContent)
  } else {
    sections.push('## Repo rules (from Supabase)')
    const apiResult = await loadInstructionsFromApi(sections, config, rulesPath)
    if (apiResult.ticketTemplateContent) {
      ticketTemplateContent = apiResult.ticketTemplateContent
    }
    if (apiResult.checklistContent) {
      checklistContent = apiResult.checklistContent
    }
  }

  addTicketTemplateSection(sections, localLoaded, ticketTemplateContent, checklistContent)
...
async function loadInstructionsFromHalApi(
  ...
  return { basicInstructions, situationalInstructions }
}

async function tryLoadInstructionsFromHalApi(
  sections: string[],
  config: PmAgentConfig
): Promise<{ bootstrapLoaded: boolean; ticketTemplateContent: string | null; checklistContent: string | null }> {
  const result = {
    bootstrapLoaded: false,
    ticketTemplateContent: null as string | null,
    checklistContent: null as string | null,
  }

  const halBaseUrl = await loadHalBaseUrl(config.repoRoot)
  if (!halBaseUrl) {
    return result
  }
  ...
  try {
    ...
    const { basicInstructions, situationalInstructions } = await loadInstructionsFromHalApi(
      halBaseUrl,
      repoFullName
    )

    result.bootstrapLoaded = appendInstructionBootstrap(
      sections,
      'HAL API',
      basicInstructions,
      situationalInstructions,
    ...
    if (templateInst?.contentMd) result.ticketTemplateContent = templateInst.contentMd
    if (checklistInst?.contentMd) result.checklistContent = checklistInst.contentMd
  } catch (apiErr) {
...
async function tryLoadEntryPointFallback(
  sections: string[],
  rulesPath: string
): Promise<void> {
  try {
...
async function loadInstructionsFromApi(
  sections: string[],
  config: PmAgentConfig,
  rulesPath: string
  ...
  try {
    const halResult = await tryLoadInstructionsFromHalApi(sections, config)
    ticketTemplateContent = halResult.ticketTemplateContent
    checklistContent = halResult.checklistContent

    if (!halResult.bootstrapLoaded) {
      await tryLoadEntryPointFallback(sections, rulesPath)
    }
  } catch (err) {
