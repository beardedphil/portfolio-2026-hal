  ...
  try {
    ...
    if (halBaseUrl) {
      try {
        ...
        if (templateInst?.contentMd) localRules.ticketTemplateContent = templateInst.contentMd
        if (checklistInst?.contentMd) localRules.checklistContent = checklistInst.contentMd
      } catch (apiErr) {
...
export async function buildContextPack(config: PmAgentConfig, userMessage: string): Promise<string> {
  ...
  if (!localRules.localLoaded) {
    await loadInstructionBootstrap(config, rulesPath, sections, localRules)
  }

  if (!localRules.localLoaded && USE_MINIMAL_BOOTSTRAP) {
    sections.push(
    ...
    if (localRules.ticketTemplateContent) {
      sections.push(
    ...
    if (localRules.checklistContent) {
      sections.push(localRules.checklistContent)
    } else {
      sections.push(
