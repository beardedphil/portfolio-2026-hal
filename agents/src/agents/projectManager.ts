...
import type { ReadyCheckResult } from '../lib/projectManagerHelpers.js'
...
import { COL_UNASSIGNED, COL_TODO } from './projectManager/toolHelpers.js'
import { createTicketTools } from './projectManager/tools/ticketTools.js'
import { createKanbanTools } from './projectManager/tools/kanbanTools.js'
import { createRedTools } from './projectManager/tools/redTools.js'
import { createRepositoryTools } from './projectManager/tools/repositoryTools.js'
import { createInstructionTools } from './projectManager/tools/instructionTools.js'
...
export { respond } from './projectManager/responseHandling.js'
export type { ConversationTurn, PmAgentConfig }
export type { WorkingMemory }
export { summarizeForContext, generateWorkingMemory } from './projectManager/summarization.js'
...
export async function runPmAgent(
  ...
  if (typeof console !== 'undefined' && console.warn) {
    console.warn(
      `[PM Agent] hasGitHubRepo=${hasGitHubRepo}, repoFullName=${config.repoFullName || 'NOT SET'}, hasGithubReadFile=${typeof config.githubReadFile === 'function'}, hasGithubSearchCode=${typeof config.githubSearchCode === 'function'}`
    )
  }

  // Create tools using factory functions
  const ticketTools = createTicketTools({
    toolCalls,
    halBaseUrl,
    config: { projectId: config.projectId, abortSignal: config.abortSignal, onProgress: config.onProgress },
    isAbortError,
  })
  ...
  const redTools = createRedTools({
    toolCalls,
    halBaseUrl,
    config: { abortSignal: config.abortSignal, onProgress: config.onProgress },
    isAbortError,
  })
  ...
  const instructionTools = createInstructionTools({
    toolCalls,
    config: { repoRoot: config.repoRoot, repoFullName: config.repoFullName, projectId: config.projectId },
  })

  // Combine all tools from factory functions
  const tools = {
    ...instructionTools,
    ...repositoryTools,
    ...ticketTools,
