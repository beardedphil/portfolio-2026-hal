...
import {
  loadInstructionSetByAgentType,
  loadInstructionTopicById,
} from './projectManager/instructionLoading.js'
import {
  buildPromptText,
  buildPromptForModel,
...
export async function runPmAgent(
  ...
  const getInstructionSetTool = tool({
    ...
    execute: async (input) => {
      try {
        ...
        if (input.agentType) {
          const result = await loadInstructionSetByAgentType(config, input.agentType)
          if ('error' in result) {
        ...
        const result = await loadInstructionTopicById(config, topicId)
        toolCalls.push({ name: 'get_instruction_set', input, output: result })
        return result
      } catch (err) {
        const error = { 
  ...
  const tools = {
    ...
    evaluate_ticket_ready: ticketTools.evaluate_ticket_ready,
  ...
  const fullPromptText = buildPromptText(promptBase, PM_SYSTEM_INSTRUCTIONS, config)
  ...
  const prompt = buildPromptForModel(promptBase, config)
