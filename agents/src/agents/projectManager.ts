...
import { createGetInstructionSetTool } from './projectManager/tools/getInstructionSet.js'
import {
  listDirectory,
  readFile,
  searchFiles,
...
export async function runPmAgent(
      ...
      try {
        ...
        if (existing?.success && Array.isArray(existing.red_versions) && existing.red_versions.length > 0) {
          ...
          try {
            ...
            const artifactVersion = vNum || redDoc?.version || 0
            const artifactTitle = createRedArtifactTitle(artifactVersion, createdAt)
            const artifactBody = createRedArtifactBody(artifactVersion, String(latest.red_id), createdAt, validationStatus, redJsonForArtifact)
              await halFetchJson(
        ...
        if (!created?.success || !created?.red_document) {
          ...
          try {
            ...
            const artifactTitle = createRedArtifactTitle(version, createdAt)
            const artifactBody = createRedArtifactBody(version, String(savedRED?.red_id ?? ''), createdAt, validationStatus, redJsonForArtifact)
            await halFetchJson(
              '/api/artifacts/insert-implementation',
              { ticketId: ticketPk, artifactType: 'red', title: artifactTitle, body_md: artifactBody },
              { timeoutMs: 25_000, progressMessage: `Saving RED artifact for ${input.ticket_id}…` }
  ...
  const getInstructionSetTool = createGetInstructionSetTool(config, halBaseUrl, toolCalls)
