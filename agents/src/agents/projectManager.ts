...
} from './projectManager/toolImplementations.js'
...
export async function runPmAgent(
  ...
  const createTicketTool = tool({
    ...
    execute: async (input: { title: string; body_md: string }) => {
      try {
        return await createTicketLogic(
          input,
          config,
          halFetchJson,
          toolCalls,
        ...
        const out = { success: false, error: err instanceof Error ? err.message : String(err) }
  ...
  const updateTicketBodyTool = tool({
    ...
    execute: async (input: { ticket_id: string; body_md: string }) => {
      try {
        return await updateTicketBodyLogic(
          input,
          halFetchJson,
          toolCalls,
          isAbortError
  ...
  const createRedDocumentTool = tool({
    ...
    parameters: jsonSchema<{ ticket_id: string; red_json_content: string }>({
      type: 'object',
      additionalProperties: false,
      properties: {
        ticket_id: {
    ...
    execute: async (input: { ticket_id: string; red_json_content: string }) => {
      try {
        return await createRedDocumentLogic(
          input,
          halFetchJson,
          toolCalls,
          isAbortError
