...
import { halFetchJson as halFetchJsonHelper } from './projectManager/halApiHelpers.js'
import { buildImageInfo, buildFullPromptText, buildPrompt } from './projectManager/promptHelpers.js'
...
export async function runPmAgent(
  ...
  const createTicketTool = createTicketToolFactory(halFetchJson, toolCalls, config, isAbortError)

  const fetchTicketContentTool = createFetchTicketContentTool(halFetchJson, toolCalls, isAbortError)
  ...
  const updateTicketBodyTool = createUpdateTicketBodyTool(halFetchJson, toolCalls, isAbortError)
  ...
  const createRedDocumentTool = createRedDocumentToolFactory(halFetchJson, toolCalls)
  ...
  const imageInfo = buildImageInfo(config)
  const fullPromptText = buildFullPromptText(PM_SYSTEM_INSTRUCTIONS, contextPack, message, imageInfo)
