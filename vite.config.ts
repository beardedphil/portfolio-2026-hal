...
export default defineConfig({
  ...
  plugins: [
    ...
    {
      name: 'artifacts-check-gate-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/artifacts/check-gate' || (req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }
          try {
            const handler = await import('./api/artifacts/check-gate.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
