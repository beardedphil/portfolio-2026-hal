...
export default defineConfig({
  plugins: [
    ...
    {
      name: 'tool-calls-check-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/tool-calls/check' || (req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }

          try {
            const handler = await import('./api/tool-calls/check.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
    ...
    {
      name: 'tool-calls-execute-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/tool-calls/execute' || (req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }

          try {
            const handler = await import('./api/tool-calls/execute.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
    ...
    {
      name: 'tool-calls-recent-executions-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/tool-calls/recent-executions' || (req.method !== 'GET' && req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }

          try {
            const handler = await import('./api/tool-calls/recent-executions.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
    ...
    {
      name: 'tool-calls-check-all-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/tool-calls/check-all' || (req.method !== 'GET' && req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }

          try {
            const handler = await import('./api/tool-calls/check-all.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
    ...
    {
      name: 'tool-calls-execute-all-endpoint',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url !== '/api/tool-calls/execute-all' || (req.method !== 'POST' && req.method !== 'OPTIONS')) {
            next()
            return
          }

          try {
            const handler = await import('./api/tool-calls/execute-all.js')
            await handler.default(req, res)
          } catch (err) {
            res.statusCode = 500
            res.setHeader('Content-Type', 'application/json')
