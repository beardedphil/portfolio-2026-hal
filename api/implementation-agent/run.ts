...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const promptText = [
      ...
      '   - **MANDATORY:** This artifact is required on EVERY ticket, without exception.',
      '   - **Content must be non-empty:** The Changed Files artifact content must never be blank, empty, or omitted.',
      '   - **When files changed:** Must include:',
      '     - A list of file paths that were created, modified, or deleted',
      '     - A brief per-file description of what changed (one line per file)',
      '     - Example: `- src/api/tickets.ts — Added ticket syncing logic`',
      '   - **When no files changed:** Must explicitly state "No files changed." followed by a short reason, such as:',
      ...
      '   - **Process failure:** Leaving the Changed Files artifact blank, empty, or omitting it entirely is a process failure and will cause QA to fail immediately.',
      '',
