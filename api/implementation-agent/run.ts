...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const promptText = [
      ...
      '   - **When files changed:** Must include a non-empty list with:',
      '     - File paths (one per line or in a list)',
      '     - Brief per-file description of what changed (one line each)',
      '     - Example: `- src/components/Button.tsx - Added disabled state styling`',
      '   - **When no files changed:** You MUST explicitly write "No files changed." followed by a brief reason, such as:',
      '     - "No files changed. Docs-only ticket handled via Supabase updates."',
      '     - "No files changed. Investigation only; no code changes made."',
      '     - "No files changed. Repro failed; no code changes made."',
      '   - **CRITICAL:** Leaving the Changed Files artifact blank, omitting it, storing it with only a title, or using placeholder text is a PROCESS FAILURE and will cause QA to fail immediately.',
      '   - The Changed Files content must be NON-EMPTY and substantive in all cases. Empty artifacts or placeholders are rejected by the HAL API.',
      '',
      ...
      '   - Must include: List of instruction sets or rules that were referenced during implementation',
      '   - Include any agent instructions, cursor rules, or process documents that guided your work',
      '   - If no specific instructions were used beyond standard agent behavior, state that explicitly',
      '',
