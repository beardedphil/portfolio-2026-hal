 ...
 * Returns the 4-digit ticket ID or null if not found.
 */
...
/**
 * Extracts goal, deliverable, and criteria sections from ticket markdown body.
 */
export function extractTicketSections(bodyMd: string): {
  goal: string
  deliverable: string
  criteria: string
} {
  ...
  return { goal, deliverable, criteria }
}

/**
 * Validates required inputs for implementation agent.
 * Returns validation result with error message and status if invalid.
 */
export function validateInputs(
...
): { valid: boolean; error?: string; status?: string } {
  if (!ticketId) {
    return {
      valid: false,
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    return {
      valid: false,
      error: `Supabase not configured. Connect project to fetch ticket ${ticketId} from Supabase.`,
  ...
  if (!repoFullName) {
    return {
      valid: false,
      error: 'No GitHub repo connected. Use "Connect GitHub Repo" first.',
      status: 'no-repo',
    }
  }
...
/**
 * Builds the implementation agent prompt text from ticket data and configuration.
 */
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const validation = validateInputs(message, ticketId, supabaseUrl, supabaseAnonKey, repoFullName)
    if (!validation.valid) {
      res.statusCode = 200
      res.setHeader('Content-Type', 'application/x-ndjson')
      writeStage({ stage: 'failed', error: validation.error, status: validation.status })
      res.end()
      return
    }
    ...
    // Extract ticket sections and build prompt
