...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const promptText = [
      ...
      '   - **MANDATORY:** This artifact MUST be created on every ticket, even if no files changed.',
      '   - **Content must be non-empty:** The Changed Files artifact content cannot be blank, empty, or omitted. This is a process requirement.',
      '   - **When files changed:** Must include:',
      '     - A list of all file paths that were created, modified, or deleted',
      '     - A brief one-line description of what changed in each file (e.g., "Added validation logic", "Updated API endpoint", "Removed deprecated function")',
      '   - **When no files changed:** Must explicitly state:',
      '     - `No files changed.`',
      '     - A brief reason explaining why (e.g., "Docs-only ticket handled via Supabase updates", "Investigation only; no code changes made", "Repro failed; no code changes made")',
