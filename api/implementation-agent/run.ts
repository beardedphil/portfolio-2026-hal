...
export function parseTicketId(message: string): string | null {
  const match = message.match(/implement\s+ticket\s+(\d{4})/i)
  return match ? match[1] : null
}

/**
 * Extracts model name from request body or environment variables.
 * Priority: body.model > CURSOR_IMPLEMENTATION_MODEL > CURSOR_AGENT_MODEL > empty string.
 */
export function getModel(body: { model?: string }): string {
  if (typeof body.model === 'string' && body.model.trim()) {
    return body.model.trim()
  }
  return (
    process.env.CURSOR_IMPLEMENTATION_MODEL?.trim() ||
...
/**
 * Extracts sections from ticket markdown body.
 * Returns goal, deliverable, and criteria as trimmed strings.
 */
