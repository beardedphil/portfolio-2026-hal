...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const promptText = [
      ...
      '',
      '2. **Check for QA artifacts** - Use the `get_artifacts` tool to fetch all artifacts for this ticket. Look for QA reports (agent_type: "qa") that may contain failure reasons or feedback.',
      '',
      ...
      '',
      '**Required artifacts (use `insert_implementation_artifact` for each):**',
      '',
      ...
      '',
      '**Failure to store all required artifacts will cause QA to fail immediately.**',
      '',
      ...
      '**Available tools:**',
      '',
      '1. **`insert_implementation_artifact`** - Store implementation artifact (MANDATORY - store all 6 required artifacts)',
      ...
      '   - **MANDATORY:** Store ALL 6 required artifacts before marking ticket ready for QA',
      '',
      '2. **`get_artifacts`** - Fetch all artifacts for a ticket (use to check for QA reports and failure notes)',
      '   - Params: `{ ticketId: string }`',
      '   - Returns: Array of artifacts including QA reports',
      '   - **Use this BEFORE starting implementation to check for previous failures**',
      '',
      '3. **`get_ticket_content`** - Fetch ticket content',
      '   - Params: `{ ticketId: string }`',
      '   - Use to get the latest ticket body if needed',
      '',
      '4. **`update_ticket_body`** - Update ticket body',
      '   - Params: `{ ticketId: string, body_md: string }`',
      '   - Use to add branch name, merge notes, etc.',
      '',
      '5. **`move_ticket_column`** - Move ticket to different column',
      '   - Params: `{ ticketId: string, columnId: string }`',
      '',
      '**Example workflow:**',
      '',
      '```',
      '1. First, check for previous failures:',
      '',
      '{',
      '  "tool": "get_artifacts",',
      '  "params": {',
      ...
      '}',
      '',
      '2. After reading QA reports, create your plan:',
      '',
      '{',
      ...
      '}',
      '',
      '3. Continue storing all 6 required artifacts as you work...',
      '```',
      '',
