 ...
 * Uses the resolution plan from _resolveTicketRef.ts to determine lookup attempts.
 */
export async function fetchTicketByPkOrId(
  ...
  for (const attempt of attempts) {
    ticketFetch = await supabase
      .from('tickets')
      .select('pk, repo_full_name, kanban_column_id, kanban_position')
      .eq(attempt.type === 'id' ? 'id' : 'display_id', attempt.value)
      .maybeSingle()

    // If we got data (success), return it
    if (ticketFetch && !ticketFetch.error && ticketFetch.data) {
      return ticketFetch
    }
    
    // If there was an error or no data, continue to next attempt
    // (matching original behavior which continues even on errors)
  }

  // Return the last attempt result (which will have error or no data)
  return ticketFetch
}
