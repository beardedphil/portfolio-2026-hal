...
import type { AgentType } from './launch.js'

/**
 * Parses agent type from request body, defaulting to 'implementation'.
 */
export function parseAgentType(bodyAgentType?: AgentType): AgentType {
...
/**
 * Extracts ticket body sections (goal, deliverable, criteria) from markdown.
 */
 ...
 * Matches patterns like "Branch: `ticket/0046-implementation`" or "Branch: ticket/0046-implementation"
 */
export function extractBranchNameFromBody(bodyMd: string): string | null {
  const branchMatch = bodyMd.match(/##\s*QA[^\n]*\n[\s\S]*?Branch[:\s]+([^\n]+)/i)
  if (!branchMatch) return null
  // Remove backticks and trim
  const branchName = branchMatch[1].replace(/`/g, '').trim()
  return branchName || null
}

/**
 * Generates branch name for implementation agent based on ticket number.
 */
...
/**
 * Appends existing PR information to prompt text if a PR is already linked.
 */
export function appendExistingPrInfo(promptText: string, existingPrUrl: string): string {
