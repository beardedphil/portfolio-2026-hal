...
/**
 * Parse agent type from request body, defaulting to 'implementation' if not specified.
 */
export function parseAgentType(body: { agentType?: AgentType }): AgentType {
...
/**
 * Parse ticket body markdown to extract goal, deliverable, and acceptance criteria.
 */
...
/**
 * Extract branch name from ticket body for QA agents.
 */
export function extractBranchName(bodyMd: string): string | null {
  const branchMatch = bodyMd.match(/##\s*QA[^\n]*\n[\s\S]*?Branch[:\s]+([^\n]+)/i)
  return branchMatch?.[1]?.trim() || null
}

/**
 * Build prompt text for implementation agents.
 */
...
/**
 * Build prompt text for QA agents.
 */
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const agentType = parseAgentType(body)
