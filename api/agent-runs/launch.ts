...
/**
 * Moves QA ticket from QA column to Doing column when QA agent starts.
 */
...
async function updateRunStage(
  ...
  const progressMessage = message || `Stage: ${stage}`
  await supabase
    .from('hal_agent_runs')
    .update({
      current_stage: stage,
      progress: appendProgress(progress, progressMessage),
    })
    .eq('run_id', runId)
}

/**
 * Gets existing PR URL for a ticket if one exists.
 */
...
/**
 * Ensures repository has at least one branch, creating initial commit if needed.
 */
...
/**
 * Launches Cursor agent and returns agent ID and status.
 */
async function launchCursorAgent(
  ...
  return { id: cursorAgentId, status: cursorStatus }
}
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const promptText =
      agentType === 'implementation'
        ? buildImplementationPrompt(repoFullName, ticketNumber!, displayId, currentColumnId, defaultBranch, halApiBaseUrl, sections)
        : buildQAPrompt(repoFullName, ticketNumber!, displayId, currentColumnId, defaultBranch, halApiBaseUrl, sections)
    ...
    const bootstrapResult = await ensureRepoHasBranch(repoFullName, defaultBranch, ghToken)
    if (bootstrapResult.error) {
      await supabase
        ...
        .update({
          ...
          error: `Repository has no branches and initial commit failed: ${bootstrapResult.error}. Ensure you have push access and try again.`,
          progress: appendProgress(initialProgress, `Bootstrap failed: ${bootstrapResult.error}`),
    ...
    const existingPrUrl = agentType === 'implementation' && ticketPk ? await getExistingPrUrl(supabase, ticketPk) : null

    const launchResult = await launchCursorAgent(promptText, repoFullName, defaultBranch, agentType, branchName, existingPrUrl, model || undefined)
    ...
    const cursorAgentId = launchResult.id
