...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (policy.status !== 'trial') {
      json(res, 200, {
        success: false,
        error: `Policy must be in trial to promote. Current status: ${policy.status}`,
      })
      return
    }
    ...
    // Update policy status to 'promoted'
    ...
    if (auditError) {
      console.error('[api/policies/promote] Failed to create audit log:', auditError)
    ...
    console.error('[api/policies/promote] Error:', err)
