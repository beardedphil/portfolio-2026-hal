...
async function handleWebRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('[api/policies/list] POST', msg, err)
...
export default async function handler(req: IncomingMessage | Request, res?: ServerResponse) {
  ...
  if (!res) {
    console.error('[api/policies/list] response object missing (received only one argument)')
    throw new Error('Policies list: response object missing')
  }
  try {
    if (req.method !== 'POST') {
      res.statusCode = 405
    ...
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[api/policies/list]', msg, err)
