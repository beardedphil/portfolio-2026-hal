...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const policyId = typeof body.policyId === 'string' ? body.policyId.trim() || undefined : undefined
    const policyKey = typeof body.policyKey === 'string' ? body.policyKey.trim() || undefined : undefined

    if (!policyId && !policyKey) {
      json(res, 400, {
        success: false,
        error: 'policyId or policyKey is required.',
      })
      return
    }
