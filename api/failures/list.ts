...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    const body = (await readJsonBody(req)) as {
      ...
      offset?: number
      failureType?: string
      orderBy?: 'last_seen_at' | 'recurrence_count' | 'first_seen_at'
    ...
    const failureType = typeof body.failureType === 'string' ? body.failureType.trim() || undefined : undefined
    const orderBy = body.orderBy || 'last_seen_at'
    ...
    // Filter by failure type if provided
    if (failureType) {
    ...
    if (failureType) {
      countQuery = countQuery.eq('failure_type', failureType)
    }
