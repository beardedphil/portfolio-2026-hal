...
import { validateQaArtifactContent } from '../_validation.js'
import {
...
export interface InsertQaArtifactParams {
  ticketId: string
  title: string
  body_md: string
}

export interface InsertQaArtifactResult {
  success: boolean
  artifact_id?: string
...
/**
 * Inserts or updates a QA artifact.
 * Handles duplicate cleanup, validation, and race conditions.
 */
export async function insertQaArtifact(
  supabase: SupabaseClient,
  params: InsertQaArtifactParams
): Promise<InsertQaArtifactResult> {
  ...
  const contentValidation = validateQaArtifactContent(params.body_md, canonicalTitle)
  ...
  if (!deleteResult.success && deleteResult.error) {
    ...
    console.warn(`[agent-tools] Failed to delete empty QA artifacts: ${deleteResult.error}`)
  }
  ...
  if (targetArtifactId) {
    ...
    if (deleteError) {
      ...
      console.warn(`[agent-tools] Failed to delete duplicate QA artifacts: ${deleteError}`)
    }
